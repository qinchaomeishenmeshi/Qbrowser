...
function getDyAccountNo() {
  ...
  const accountNo = accountNoEle.querySelector(
    "div[class*='panel-profile-account-']"
  ).textContent;
  const accountName = accountNoEle.querySelector(
...
async function _get_live_history_list() {
  ...
  // 周期天数
  const periodDays = 7;
  const { begin_date, begin_date_format } = getBeginDate(periodDays);
  // 非直播明细页面需要先跳转到直播明细，否则直播大屏权限不足
  if (window.location.pathname !== "/dashboard/compass-home/live-list") {
    window.location.replace(
      "https://buyin.jinritemai.com/dashboard/compass-home/live-list"
    );
  }
  try {
    ...
    const res = await fetch(
      `https://buyin.jinritemai.com/compass_api/content_live/author/live_detail/history_live?is_asc=false&page_no=1&page_size=10&date_type=21&begin_date=${begin_date}&begin_date_format=${begin_date_format}`,
      {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      }
    ...
    function openLinksSequentially(items, index = 0) {
      ...
      if (item.operation?.live_id) {
        window.open(
          `https://compass.jinritemai.com/screen/live/talent?live_room_id=${item.operation.live_id}`
        );
      }
...
async function _get_live_core_data(postData) {
  try {
    ...
    workTimeCallBack(() => {
      closeTabByUrl(
        "https://compass.jinritemai.com/screen/live/talent?live_room_id=" +
          postData.live_id
      );
    }, 2000);
  }
