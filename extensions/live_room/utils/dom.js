...
async function waitForElement(selector, options = {}) {
    const {isAll = false, timeout = 5 * 60 * 1000, parentElement = document, isNeedReject = true} = options
...
async function simulateWheelEvent(list, target, retryCount = 0, maxRetries = 5) {
    return new Promise(async (resolve, reject) => {
        try {
            // 查找目标元素
            const targetElement = Array.from(list).find((li) => li.textContent.trim().includes(target))
            ...
            const checkPosition = async () => {
                ...
                if (isAtPosition) {
                    ...
                    if (retryCount < maxRetries) {
                        ...
                        setTimeout(async () => {
                            await simulateWheelEvent(list, target, retryCount + 1, maxRetries)
                            resolve()
                        }, 500)
                    } else {
...
async function selectPublishType(isTiming) {
    ...
    // 找到所有label标签
    const labels = await waitForElement('.card-container-creator-layout label>span', {isAll: true})
...
function getCurrentDate() {
    return new Promise(async (resolve, reject) => {
        try {
            // 找到当前日期元素
            const currentDate = await waitForElement('.semi-datepicker-day.semi-datepicker-day-selected')
...
async function selectTime(hour, minute) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            // 获取要触发双击事件的 li 元素
            const hourList = await waitForElement('li', {isAll: true, parentElement: selectList[0]})
            await simulateWheelEvent(hourList, hour)
            const minuteList = await waitForElement('li', {isAll: true, parentElement: selectList[1]})
