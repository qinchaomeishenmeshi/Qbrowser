...
from conf import PORTS_FILE, BASE_DIR
...
SITE_CONFIGS = {
    "baiying": {
        ...
        "api_paths": ["/selection/common/btm_mapping", "/api/user/info"],
...
class BrowserOperator:
    ...
    def filter_data(
        ...
        # 筛选cookies
        if required_cookies and data.get("cookies"):
            for key in required_cookies:
                if key in data["cookies"]:
                    result["cookies"][key] = data["cookies"][key]
        else:
        ...
        if required_headers and all_headers:
            for key in required_headers:
                if key in all_headers:
                    result["headers"][key] = all_headers[key]
        else:
    ...
    async def collect_site_cookies(
        ...
        try:
            ...
            filtered_data = self.filter_data(
                raw_data,
                config.get("required_cookies"),
                config.get("required_headers")
            )
            
            # 检查是否获取了所需数据
            cookies_ok = all(key in filtered_data["cookies"] for key in config.get("required_cookies", []))
            headers_ok = all(key in filtered_data["headers"] for key in config.get("required_headers", []))
            
            if not cookies_ok or not headers_ok:
                missing_cookies = [k for k in config.get("required_cookies", []) if k not in filtered_data["cookies"]]
                missing_headers = [k for k in config.get("required_headers", []) if k not in filtered_data["headers"]]
                logger.warning(f"用户 {user_id} 的数据不完整，缺少: cookies={missing_cookies}, headers={missing_headers}")
