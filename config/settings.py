...
class NetworkConfig:
    ...
    def __post_init__(self):
        ...
        self.browser_start_port = int(
            os.getenv("BROWSER_START_PORT", self.browser_start_port)
        )
...
class BrowserConfig:
    ...
    disable_features: List[str] = field(
        default_factory=lambda: ["VizDisplayCompositor", "TranslateUI"]
    )
...
class SchedulerConfig:
    ...
    executors: Dict[str, Any] = field(
        default_factory=lambda: {"default": {"type": "threadpool", "max_workers": 4}}
    )
...
class SecurityConfig:
    ...
    def __post_init__(self):
        ...
        self.jwt_expire_hours = int(
            os.getenv("JWT_EXPIRE_HOURS", self.jwt_expire_hours)
        )
        self.rate_limit_per_minute = int(
            os.getenv("RATE_LIMIT_PER_MINUTE", self.rate_limit_per_minute)
        )
...
class AppSettings:
    ...
    def validate(self) -> List[str]:
        ...
        if not (1024 <= self.network.browser_start_port <= 65535):
            errors.append(
                f"浏览器起始端口超出有效范围: {self.network.browser_start_port}"
            )
