...
def main():
    ...
    try:
        ...
        if len(sys.argv) > 1 and sys.argv[1] == "--help":
            print("用法: python jianying.py [配置文件路径=config.json]")
            print("配置文件格式示例:")
        ...
        all_results = searcher.process_multiple_keywords()
        
        # 打印结果摘要
        print("\n处理结果摘要:")
        for keyword, result in all_results.items():
            if "error" in result:
                print(f"关键词 '{keyword}' 处理失败: {result['error']}")
            else:
                print(f"关键词 '{keyword}' 处理成功:")
                if 'full_result_path' in result:
                    print(f"  - 完整搜索结果: {result['full_result_path']}")
                if 'video_urls_path' in result:
                    print(f"  - 视频URL: {result['video_urls_path']}")
                if 'downloaded_videos' in result:
                    print(f"  - 已下载 {len(result['downloaded_videos'])} 个视频")
