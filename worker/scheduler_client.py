...
class SchedulerClient:
    ...
    async def remove_task(self, task_id: str) -> bool:
        ...
        try:
            ...
            await db.execute("DELETE FROM task_configs WHERE task_id = ?", (task_id,))
            await db.commit()
    ...
    async def update_all_task_configs(self):
        ...
        try:
            ...
            logger.info(
                f"获取到 {len(device_ids)} 个设备，将用于更新任务配置: {device_ids_str}"
            )
    ...
    async def _execute_task(self, task_config: TaskConfig):
        """执行任务"""
        execution_id = (
            f"{task_config.task_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    async def _get_active_device_list(self) -> List[str]:
        ...
        try:
            ...
            active_devices = [
                manager.user_id for manager in managers if manager.is_running
            ]
            logger.info(
                f"获取到 {len(active_devices)} 个活跃浏览器实例: {active_devices}"
            )
    ...
    def _validate_task_config(self, task_config: TaskConfig) -> bool:
        ...
        if task_config.target_function not in [
            "get_replay_punish_list_main",
            "get_live_room_list_main",
            "get_core_data_main",
            "get_history_live_main",
        ]:
    ...
    async def _load_configs(self):
        """通过 DatabaseManager 加载配置"""
        try:
            db = await db_manager.get_db()
            async with db.execute("SELECT * FROM task_configs") as cursor:
                ...
                for row in rows:
                    config_dict = {
                        ...
                        "function_params": json.loads(row["function_params_json"]),
                        "enabled": bool(row["enabled"]),
                        "max_instances": row["max_instances"],
                        "created_at": row["created_at"],
                        "updated_at": row["updated_at"],
                    }
                    self.task_configs[row["task_id"]] = TaskConfig(**config_dict)
            logger.info(f"从数据库加载了 {len(self.task_configs)} 个任务配置")
        except Exception as e:
        ...
        try:
            ...
            for config in self.task_configs.values():
                await db.execute(
                    ...
                    (task_id, name, description, trigger_type, trigger_config_json, 
                     target_function, function_params_json, enabled, max_instances, 
                    ...
                    (
                        ...
                        json.dumps(config.trigger_config, ensure_ascii=False),
                        config.target_function,
                        json.dumps(config.function_params, ensure_ascii=False),
    ...
    async def _load_results(self):
        ...
        try:
            ...
            async with db.execute(
                "SELECT * FROM task_results ORDER BY start_time DESC LIMIT 1000"
            ) as cursor:
                rows = await cursor.fetchall()
                for row in rows:
                    result_dict = {
                        ...
                        "result_data": (
                            json.loads(row["result_data_json"])
                    ...
                    self.task_results.append(TaskResult(**result_dict))
            logger.info(f"从数据库加载了 {len(self.task_results)} 个任务结果")
        except Exception as e:
            ...
            for result in self.task_results[-10:]:
                await db.execute(
                    ...
                    (execution_id, task_id, status, start_time, end_time, 
                     duration, result_data_json, error_message, retry_count) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        result.execution_id,
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                ...
                async with session.post(
                    url=url, json=data, headers=self.default_headers
                ) as response:
            ...
            return {"status": "failed", "message": f"未知错误: {str(e)}", "data": None}
    ...
    async def main():
        ...
        try:
            ...
            await create_cron_task(
                ...
                function_params={"user_id": "001", "room_id": "7512002735768865571"},
            )
